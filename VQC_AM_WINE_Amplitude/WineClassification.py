...
# Append 3 zeros to each vector (we want vectors to be 16 elements long, currently 13 elements)
zeros_to_append_train = torch.zeros((X_train.shape[0], 3))  # 3 zeros for each sample
X_train = torch.cat((X_train, zeros_to_append_train), dim=1)

zeros_to_append_test = torch.zeros((X_test.shape[0], 3))  # 3 zeros for each sample
X_test = torch.cat((X_test, zeros_to_append_test), dim=1)
